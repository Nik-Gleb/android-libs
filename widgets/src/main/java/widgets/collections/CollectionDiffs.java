/*
 * 	DiffsCalculator.java
 * 	internal-common
 *
 * 	Copyright (C) 2017, Webzilla Apps Inc. All Rights Reserved.
 *
...
/**
 * CollectionDiffs is a utility class that can calculate the difference between two lists and output a
 * list of update operations that converts the first list into the second one.
 * <p>
 * It can be used to calculate updates for a RecyclerView Adapter.
 * <p>
 * CollectionDiffs uses Eugene W. Myers's difference algorithm to calculate the minimal number of updates
 * to convert one list into another. Myers's algorithm does not handle items that are moved so
 * CollectionDiffs runs a second pass on the result to detect items that were moved.
 * <p>
 * If the lists are large, this operation may take significant time so you are advised to run this
 * on a background thread, get the {@link DiffsResult} then apply it on the RecyclerView on the main
 * thread.
 * <p>
...
public final class CollectionDiffs {
    ...
    /** The log-cat tag. */
    private static final String TAG = "CollectionDiffs";

    /** The snake comparator */
    private static final SnakeComparator SNAKE_COMPARATOR = new SnakeComparator();
     ...
     **/
    private CollectionDiffs() {throw new AssertionError();}

    // Myers' algorithm uses two lists as axis labels. In CollectionDiffs's implementation,
    // `x` axis is used for old list and `y` axis is used for new list.
    ...
    @NonNull public static <T> DiffsResult calc(@NonNull T[] oldItems, @NonNull T[] newItems)
    {return calc(oldItems, newItems, true);}
     ...
     *
     * @param detectMoves True if CollectionDiffs should try to detect moved items, false otherwise.
     *
     * @return A DiffResult that contains the information about the edit sequence to convert the
     * old list into the new list.
     */
    @NonNull public static <T> DiffsResult calc
    (@NonNull T[] oldItems, @NonNull T[] newItems, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            final Snake snake = diffPartial(oldItems, newItems,
            ...
            if (snake != null) {
                ...
                stack.add(right);
            } else rangePool.add(range);
        }

        // sort snakes
        snakes.sort(SNAKE_COMPARATOR);

        return new DiffsResult<>(oldItems, newItems, snakes, forward, backward, detectMoves);

    }

    private static <T> Snake diffPartial(@NonNull T[] oldItems, @NonNull T[] newItems, int startOld, int endOld,
        ...
        for (int d = 0; d <= dLimit; d++) {
            for (int k = -d; k <= d; k += 2) {
                ...
                while (x < oldSize && y < newSize &&
                    oldItems[startOld + x].hashCode() ==
                    newItems[startNew + y].hashCode())
                {x++; y++;}
                forward[kOffset + k] = x;
            ...
            for (int k = -d; k <= d; k += 2) {
                ...
                // move diagonal as long as items match
                while (x > 0 && y > 0 &&
                    oldItems[startOld + x - 1].hashCode() ==
                    newItems[startNew + y - 1].hashCode())
                {x--; y--;}
        ...
        throw new IllegalStateException("CollectionDiffs hit an unexpected case while trying to calculate"
    ...
    private static final class Snake {
        ...
        /** Position in the old list. */ int x;
        /** Position in the new list. */ int y;
        /** Number of matches. Might be 0. */ int size;
    ...
    private static final class Range {
        Range() {}
        int oldListStart, oldListEnd, newListStart, newListEnd;
        Range(int oldListStart, int oldListEnd, int newListStart, int newListEnd) {
            this.oldListStart = oldListStart; this.oldListEnd = oldListEnd;
            this.newListStart = newListStart; this.newListEnd = newListEnd;
        }
    }

    /** Snake comparator class */
    private static final class SnakeComparator implements Comparator<Snake> {
        /** {@inheritDoc} */
        @Override public final int compare(Snake o1, Snake o2)
        {final int cmpX = o1.x - o2.x; return cmpX == 0 ? o1.y - o2.y : cmpX; }
    }

    /**
     * This class holds the information about the result of a calc call.
     * <p>
     * You can consume the updates in a DiffsResult via {@link #dispatch(ListUpdateCallback)}
     *
     * @author Nikitenko Gleb
     * @since 1.0, 07/01/2017
     */
    @Keep
    @KeepPublicProtectedClassMembers
    public static final class DiffsResult<T> {
        ...
        /** The payloads. */
        private final ArrayMap<PayloadKey, T> mPayloads = new ArrayMap<>();
         ...
         */
        private DiffsResult(@NonNull T[] oldItems, @NonNull T[] newItems,
            @NonNull List<Snake> snakes, int[] oldItemStatuses, int[] newItemStatuses,
            boolean detectMoves) {
            ...
            mOldListSize = oldItems.length;
            mNewListSize = newItems.length;
            mDetectMoves = detectMoves;

            addRootSnake(); findMatchingItems(oldItems, newItems);
        }
         ...
         * <p>
         * CollectionDiffs does this pre-processing so that if it is running on a big list, it can be moved
        ...
        (@NonNull T[] oldItems, @NonNull T[] newItems) {
            ...
            for (int i = mSnakes.size() - 1; i >= 0; i--) {
                ...
                if (mDetectMoves) {
                    while (posOld > endX) {
                        ...
                        findAddition(oldItems, newItems, posOld, posNew, i);
                        posOld--;
                    }
                    while (posNew > endY) {
                        // this is an addition. Check remaining snakes to see if this was removed
                        // before
                        findRemoval(oldItems, newItems, posOld, posNew, i);
                        posNew--;
                    }
                }
                for (int j = 0; j < snake.size; j++) {
                    ...
                    final T newValue = newItems[newItemPos];
                    final boolean theSame = Objects.deepEquals(oldItems[oldItemPos], newValue);
                    if (!theSame) mPayloads.put(new PayloadKey(oldItemPos, newItemPos), newValue);
        ...
        private void findAddition(@NonNull T[] oldItems, @NonNull T[] newItems, int x, int y, int snakeIndex) {
            // already set by a latter item
            if (mOldItemStatuses[x - 1] != 0) return;
            findMatchingItem(oldItems, newItems, x, y, snakeIndex, false);
        }

        private void findRemoval(@NonNull T[] oldItems, @NonNull T[] newItems, int x, int y, int snakeIndex) {
            // already set by a latter item
            if (mNewItemStatuses[y - 1] != 0) return;
            findMatchingItem(oldItems, newItems, x, y, snakeIndex, true);
        }
         ...
         */
        private void findMatchingItem
        (@NonNull T[] oldItems, @NonNull T[] newItems, final int x, final int y,
            final int snakeIndex, final boolean removal) {
            ...
            for (int i = snakeIndex; i >= 0; i--) {
                ...
                if (removal) {
                    ...
                    for (int pos = curX - 1; pos >= endX; pos--) {
                        if (oldItems[pos].hashCode() == newItems[myItemPos].hashCode()) {
                            // found!
                            final T newValue = newItems[myItemPos];
                            final boolean theSame = Objects.deepEquals(oldItems[pos], newValue);
                            if (!theSame) mPayloads.put(new PayloadKey(pos, myItemPos), newValue);
                    ...
                    for (int pos = curY - 1; pos >= endY; pos--) {
                        if (oldItems[myItemPos].hashCode() == newItems[pos].hashCode()) {
                            // found
                            final T newValue = newItems[pos];
                            final boolean theSame = Objects.deepEquals(oldItems[myItemPos], newValue);
                            if (!theSame) mPayloads.put(new PayloadKey(myItemPos, pos), newValue);
                            final int changeFlag = theSame ? FLAG_MOVED_NOT_CHANGED : FLAG_MOVED_CHANGED;
         ...
         */
        public final void dispatch(ListUpdateCallback updateCallback) {
            ...
            int posOld = mOldListSize; int posNew = mNewListSize;
            for (int snakeIndex = mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--)
                    if ((mOldItemStatuses[snake.x + i] & FLAG_MASK) == FLAG_CHANGED)
                        batchingCallback.onChanged(snake.x + i, 1, getPayload(snake.x + i, snake.y + i));

                posOld = snake.x; posNew = snake.y;
            }
        ...
        private void dispatchAdditions(List<PostponedUpdate> postponedUpdates, ListUpdateCallback updateCallback,
                                       int start, int count, int globalIndex) {
            if (!mDetectMoves) {updateCallback.onInserted(start, count); return; }
            for (int i = count - 1; i >= 0; i--) {
                int status = mNewItemStatuses[globalIndex + i] & FLAG_MASK;
                switch (status) {
                    case 0: // real addition
                        updateCallback.onInserted(start, 1);
                        for (PostponedUpdate update : postponedUpdates) update.currentPos += 1;
                        break;
                    ...
                    case FLAG_MOVED_NOT_CHANGED:
                        ...
                        if (status == FLAG_MOVED_CHANGED)
                            ...
                            updateCallback.onChanged(start, 1, getPayload(pos, globalIndex + i));
                        break;
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                switch (status) {
                    ...
                    case FLAG_MOVED_NOT_CHANGED:
                        ...
                        if (status == FLAG_MOVED_CHANGED)
                            // also dispatch a change
                            updateCallback.onChanged(update.currentPos - 1, 1, getPayload(globalIndex + i, pos));
                        break;
                    case FLAG_IGNORE: // ignoring this
        ...
        /**
         * @param oldPosition old position of item
         * @param newPosition new position of item
         * @return the payload object
         */
        @NonNull private T getPayload(int oldPosition, int newPosition)
        {return mPayloads.get(new PayloadKey(oldPosition, newPosition));}
        ...
        private static class PostponedUpdate {
            final int posInOwnerList; final boolean removal; int currentPos;
            PostponedUpdate(int posInOwnerList, int currentPos, boolean removal)
            {this.posInOwnerList = posInOwnerList; this.currentPos = currentPos; this.removal = removal; }
        }

        /** The payloads map key */
        private static final class PayloadKey {
            ...
            /**
             * Constructs a new Payload Key.
             *
             * @param oldPosition old item position
             * @param newPosition new item position
             */
            PayloadKey(int oldPosition, int newPosition)
            {mOldPosition = oldPosition; mNewPosition = newPosition;}

            /** {@inheritDoc} */
            @Override public final boolean equals(Object obj) {
                if (this == obj) return true;
                if (!(obj instanceof PayloadKey)) return false;
                final PayloadKey that = (PayloadKey) obj;
                return mOldPosition == that.mOldPosition &&
                    mNewPosition == that.mNewPosition;
            }

            /** {@inheritDoc} */
            @Override public final int hashCode()
            {return Objects.hash(mOldPosition, mNewPosition);}
        }
