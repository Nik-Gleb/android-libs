...
final class AssetsProvider implements Provider {
  ...
  AssetsProvider(@NonNull Context context, @NonNull String authority, int version)
  ...
  @NonNull @Override public final Cursor query(@NonNull Uri uri,
    ...
    isCanceled(signal); uri = convert(uri);
  ...
  (@NonNull Uri uri, @NonNull String filter) {
    ...
    String[] files; try {files = mAssets.list(path);}
    catch (IOException exception) {files = null;}
    if (files == null || files.length == 0) return null;
    final Set<String> set = Arrays.stream(files)
  ...
  (@NonNull Uri uri, @NonNull String mode, @Nullable CancellationSignal signal)
      throws FileNotFoundException {return openAssetFile(uri, mode, signal).getParcelFileDescriptor();}
   ...
   */
  @NonNull private static AssetFileDescriptor openFile(@NonNull Uri uri,
  @NonNull AssetManager assets, @Nullable CancellationSignal signal) throws FileNotFoundException {
    final String path = full(convert(uri)), type = getMimeType(path);
    final Bundle extras = new Bundle(); extras.putString(TYPE, type);
    try {isCanceled(signal); final AssetFileDescriptor afd = assets.openFd(path);
      return new AssetFileDescriptor(afd.getParcelFileDescriptor(),
          afd.getStartOffset(), afd.getDeclaredLength(), extras);
    } catch (IOException exception) {
      try {return fromResponse(new AssetRequest(assets.open(path), type), commonPool());}
      catch (IOException throwable) {
        if (!FileNotFoundException.class.isAssignableFrom(throwable.getClass()))
          throw new OperationCanceledException(throwable.getMessage());
        else throw (FileNotFoundException) throwable;
      }
    }
  }
   ...
   * @return assets uri
   */
  @NonNull private static Uri convert(@NonNull Uri uri) {
   ...
   */
  @NonNull private static String full(@NonNull Uri uri)
   ...
   */
  @NonNull private static String path(@NonNull Uri uri) {
   ...
   */
  @NonNull private static String name(@NonNull Uri uri)
  ...
  private static final class AssetRequest extends ResponseBody {
     ...
     */
    AssetRequest(@NonNull InputStream stream, @NonNull String type) throws IOException
    {mSource = OkUtils.source(stream); mType = MediaType.parse(type); mLength = stream.available();}

    /** {@inheritDoc} */
    @Nullable @Override public final MediaType contentType() {return mType;}

    /** {@inheritDoc} */
    @Override public final long contentLength() {return mLength;}

    /** {@inheritDoc} */
    @Override public final BufferedSource source() {return mSource;}
  }
